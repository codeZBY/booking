<template>
  <div class="WebOrder">
    <div class="webTop">
      ...
      >{{$i18n.locale == 'zh_CN' ? 'EN' : '中文'}}</el-button>
      <span>预约服务</span>
      <div>
        <span>SUBCONSCIOUS DAY SPA</span>
      </div>
    </div>
    <el-form :model="form" :rules="rules" ref="ruleForm" label-width="70px" class="demo-ruleForm">
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-form-item :label="$t('mBooking.gender')" prop="gender">
        ...
        <el-col :span="12">
          <el-form-item :label="$t('mBooking.code')" prop="code">
            ...
            <el-button
              ...
              type="text"
              style="color:#AA942D;"
            >{{second===0 ? $t('mBooking.getCode') : $t('mBooking.surplus')+second+'S'}}</el-button>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
      ...
      <div class="customerBox" v-for="(item,index) in this.form.customerArr" :key="index">
        ...
        <el-row class="itemLine" v-for="(item22, index22) in itemArr" :key="index22">
          <el-col
            :span="4"
          >{{$i18n.locale == 'zh_CN' ? item22.ServiceName : item22.cfdServiceNameEn}}</el-col>
          <el-col :span="20">
            <el-checkbox-group v-model="item.MentDtlList">
              <el-checkbox
                style="margin:0 15px 5px 0;"
                v-for="(item2,index2) in item22.ItemClassList"
                :key="index2"
                :label="item2.ifdClassId"
              >{{$i18n.locale == 'zh_CN' ? item2.cfdItemClass : item2.cfdItemClassEn}}</el-checkbox>
            </el-checkbox-group>
          </el-col>
        </el-row>
      </div>
      ...
      <div class="text-center">
        <el-button
          ...
          style="background:#AA942D;border:none;margin-top:15px;"
        >{{$t('mBooking.submit')}}</el-button>
      </div>
    </el-form>

    <!--  -->
    <el-dialog title="提示" center :visible.sync="dialogVisible" width="666px;">
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        branch: [
          {
            required: true,
            message: this.$t("mBooking.branchPlace"),
            trigger: "change"
          }
        ],
        gender: [
          {
            required: true,
            message: this.$t("mBooking.genderPlace"),
            trigger: "change"
          }
        ],
        number: [
  ...
  methods: {
    ...
    submitData() {
      ...
      this.form.customerArr.forEach(item => {
        let pickArr = [];
      ...
      let data = {
        ...
        cfdWeixinOpenid: "",
        cfdMemberId: "",
        AppointMentNumber: this.form.number,
        MentMstList: custArr,
        ...
        .then(res => {
          ...
          if (Number(res.code) === 0) {
            ...
            if (this.$i18n.locale == "zh_CN") {
              ...
              msg =
                "We will confirm your reservation in your mailbox, please check your reservation status.";
            }
    ...
    getMailCode() {
      ...
      // this.waitSecond()
      this.$axios
        .get(this.GLOBAL.baseURL + "Common/GetSendEmailCode", {
          params: {
    ...
    addCust() {
      ...
      this.form.customerArr = JSON.parse(JSON.stringify(arr));
    },
    getShops() {
      this.$axios
